...
const SUPPORTED_AGENTS = ["claude", "codex", "copilot", "gemini", "github", "opencode"] as const;
...
async function runRustTauri(args: string[]): Promise<void> {
...
function parentOfInstallDestination(
  positional: string | undefined,
  destFlag: string | undefined,
  global: boolean,
): string {
...
function isSupportedAgent(value: string): value is AgentName {
  return (SUPPORTED_AGENTS as readonly string[]).includes(value.toLowerCase());
}
